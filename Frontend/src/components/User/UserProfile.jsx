...
const handleSave = async () => {
  ...
  const getProgressColor = (status) => {
    switch (status) {
      case "Done": return "bg-[#006A67]";
      case "In Progress": return "bg-[#F4991A]";
      case "Pending": return "bg-[#F75270]";
  ...
  const taskDueDates = new Set(tasks.map((t) => new Date(t.deadline).getDate()));
            ...
            <div className="space-y-4 text-gray-600">
              {isEditing ? (
                <div className="space-y-4">
                  ...
                  <input name="contact" value={formData.contact} onChange={handleChange} className="border p-2 w-full rounded" />
                ...
                <div className="space-y-4 text-gray-600">
                  ...
                  <button onClick={() => setIsEditing(true)} className="bg-gray-500 text-white px-4 py-2 rounded mt-2">Edit</button>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm col-span-1 md:col-span-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {tasks.length === 0 && <p className="text-gray-500">No tasks assigned yet.</p>}
              {tasks.map((task) => (
                <div key={task._id} className="p-4 rounded-xl bg-gray-50 border border-gray-100 space-y-2">
                  <div className="flex justify-between items-center mb-2">
                    <span className={`text-xs font-semibold rounded-full px-2 py-1 ${getAdminStatusStyle(task.decision || "Pending")}`}>
                      {task.decision || "Pending"}
                    </span>
                    <span className="text-xs text-gray-500 font-medium">{task.deadline ? new Date(task.deadline).toLocaleDateString() : "No date"}</span>
                  </div>
                  <h4 className="font-bold text-gray-800">{task.title}</h4>
                  <div className="flex justify-between items-center text-sm text-gray-500 mt-2">
                    <span className={`px-3 py-1 text-xs font-semibold rounded-full ${getStatusTextStyle(task.status)}`}>{task.status}</span>
                    <div className="relative">
                      <button onClick={() => toggleDropdown(task._id)} className="flex items-center text-indigo-600 hover:underline text-xs font-medium">
                        Update Status {openDropdown === task._id ? <ChevronUp className="w-3 h-3 ml-1" /> : <ChevronDown className="w-3 h-3 ml-1" />}
                      </button>
                      {openDropdown === task._id && (
                        <div className="absolute right-0 mt-2 w-40 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 z-10">
                          <div className="py-1">
                            {["In Progress", "Done", "Pending"].map((status) => (
                              <button key={status} onClick={() => handleUpdateStatus(task._id, status)} className="block px-4 py-2 text-sm text-gray-800 font-medium hover:bg-gray-100 w-full text-left">{status}</button>
                            ))}
                          </div>
                        </div>
                      )}
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm">
            ...
            <div className="grid grid-cols-7 gap-1 text-center text-sm">
              {["M", "T1", "W", "T2", "F", "S1", "S2"].map((day, i) => (
                <div key={i} className="text-xs text-gray-400">{day.replace(/[0-9]/g, "")}</div>
              ))}
              {Array.from({ length: 31 }, (_, i) => i + 1).map((day) => (
                <div key={day} className={`w-8 h-8 flex items-center justify-center rounded-full font-medium ${getDayColor(day)}`}>{day}</div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm">
            <h3 className="text-lg font-bold text-gray-800 mb-4 flex justify-between items-center">
              ...
              <div className="flex gap-1">
                ...
                />
                <button onClick={addSkill} className="bg-blue-500 text-white px-2 py-1 rounded text-sm">Add</button>
              </div>
            </h3>
            <div className="flex flex-wrap gap-2">
              {skills.map((skill) => (
                <div key={skill.id} className={`flex items-center gap-1 px-3 py-1 text-sm font-semibold rounded-full ${skill.color}`}>
